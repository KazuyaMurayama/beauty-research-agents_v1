...
3. **`FILE_INDEX.md`** または **`file_index.md`** — ファイル構成の索引

> 編集前に、ユーザー発話のキーワードを `FILE_INDEX.md` / `file_index.md` と照合してから対象ファイルを特定する（推測で着手しない）。
...
3. **報告前に URL 存在確認**: `gh api repos/KazuyaMurayama/beauty-research-agents_v1/contents/PATH?ref=main` でステータス200確認。
...
### ファイル名
- 基本形 `<TOPIC>_YYYYMMDD.md`（**サフィックス・ハイフンなし**）。例: `BEAUTY_REPORT_20260603.md`
- 同日中の追加更新は `-v2`、`-v3`（例: `BEAUTY_REPORT_20260603-v2.md`）。
- 日付が変わったら v サフィックスをリセット（例: 翌日1回目 `BEAUTY_REPORT_20260604.md`）。
